 ...
 *    `color-mix()`. This lab has both, and they carry most of its tinted
 *    surfaces: the guided-tour stage is a `linear-gradient(160deg,
 *    color-mix(in oklab, var(--accent) 11%, var(--surface)), var(--surface)
 *    62%)`, its three stage columns are `color-mix(… var(--surface-2) 78%,
 *    transparent)` painted over that gradient, the sticky exhibit navigator and
 *    the comparison panel's recommendation card are each their own
 *    `color-mix`, and the hero's "why it matters" aside is `color-mix(in oklab,
 *    var(--accent) 6%, transparent)` — a translucent tint whose real backdrop
 *    is the page background two levels up. axe files every one of those under
 *    `incomplete` rather than measuring them.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This lab has exactly one
 *    such rule, `.cl-hero-sub { opacity: .85 }` in the shared hero, and it is
 *    measured here rather than assumed harmless. The gate this replaces
 *    injected a stylesheet to suppress motion; the same injection idiom
 *    elsewhere in this fleet carried `* { opacity: 1 !important }`, which
 *    fabricates a pass for precisely this case. Nothing is injected here.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. This page is full
 *    of them: every `.hexblock` is an `overflow-x: auto` box around a 128-hex-
 *    digit group element, `.hexblock-tall` adds `overflow-y: auto` over a
 *    9rem cap on the 200-plus-digit exponent u, every `.tablewrap` scrolls the
 *    comparison and prediction tables sideways, and `.labnav-scroll` scrolls
 *    the eleven exhibit links. Content scrolled past those boxes is not dimmed
 ...
 *    load-bearing here, not defensive: the comparison panel draws a real
 *    proof-size chart whose axis labels, axis titles, direct series labels and
 *    crossover annotations are all `<text>` inside the `<svg>`, sitting over
 *    grid `<line>`s and the crossover `<circle>`s. An ancestor-only walk would
 *    composite those glyphs onto the `<svg>`'s own transparent background and
 *    report a ratio nothing on screen has; and SVG text takes its ink from
 *    `fill`, not `color`, which is the other half the walk below handles.
 */
...
/** Walk every element on the page that owns text of its own. */
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * this page — the tour stage, its columns, the exhibit navigator, the
     * recommendation card, the hero aside, the shared top bar's `--cl-ink` —
     * is authored with `color-mix()`, and Chromium reports that to
     ...
     * that is every `.hexblock` (and the taller `.hexblock-tall`), every
     * `.tablewrap`, and `.labnav-scroll`.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // ratio was 6.15:1. This chart is exactly that shape: its horizontal grid
      // rules are stroke-only `<line>`s crossing the whole plot, and its four
      // series are `<path fill="none">` — `resolve` returns null for the
      // latter, so they are skipped by a different route, and `<line>` is not
      // in FILLED so the former are skipped by this one.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. `checkVisibility()` catches it, and it matters a great
      // deal on this page: the lab ships fourteen `<details class="expert">`
      // blocks. The gate opens each one by clicking its `<summary>` and scans
      // it open AND shut, rather than setting `open = true` on all of them at
      // once and stripping every `[hidden]`, which is what the gate this
      // replaces did — producing a document no visitor is ever shown.
     ...
     * it, the `aria-hidden` decoration this lab carries — every `.verdict-icon`
     * glyph, the `→` between the tour's stage columns, the `·` separator in the
     * navigator's status line, the digit inside each tour dot (whose accessible
     * name is a sibling `.sr-only` span), and the shared header's hamburger and
     * GitHub marks — would be measured as if each took its element's declared
     * `color`. Note the boundary cuts both ways and
     ...
     *
     * The comparison chart is built with `createElementNS`, so it has no
     * stray text nodes today. The guard still earns its place: that same
     * `<svg>` carries a `<title>` and a `<desc>` whose text content is real
     * character data in the DOM and paints nothing at all — measuring either
     * would report a failure for ink that is never laid down, while the
     * accessible name they provide is exactly what makes the chart legal.
     */
